...
bool BMP180_init(uint8_t addr)
{
  ...
  return BMP180_readCalibrationCoefficients(addr);  
}
...
int32_t BMP180_getSeaLevelPressure(uint8_t addr, int16_t trueAltitude)
{
  ...
  return (pressure / (1.0 - (float)trueAltitude / 44330, 5.255));
}
...
bool BMP180_readCalibrationCoefficients(uint8_t addr)
{
  ...
  for (uint8_t reg = BMP180_CAL_AC1_REG; reg <= BMP180_CAL_MD_REG; reg=reg+2)
  {
    ...
    if (value == BMP180_ERROR) return true; //error handler, collision on i2c bus
...
int32_t BMP180_altitude(uint8_t addr)
{
 ...
 // double P0 = BMP180_getSeaLevelPressure(BMP180_ADDR, 1618 );
	...
	int32_t P = BMP180_getPressure(BMP180_ADDR);

	altitude = K * (1.0 - ((P + P0)*E));
...
uint16_t BMP180_read16(uint8_t addr, uint8_t reg)
{
  ...
  uint8_t data1  = 0;
	...
	i2cRead  ( I2C , addr, data0, 1, ON);

	i2cRead ( I2C , addr, data1, 1, ON);
...
bool BMP180_write8(uint8_t addr, uint8_t reg, uint8_t control)
{
  ...
  uint8_t data1 = 0;
	...
	state = i2cWrite ( I2C , addr,  data0, 1, OFF);
	state = i2cWrite ( I2C , addr,  data1, 1, ON);
